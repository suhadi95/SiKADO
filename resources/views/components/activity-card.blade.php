...
<article class="rounded-2xl bg-white p-3 shadow-sm ring-1 ring-slate-200/70" x-data="uploadModal">
    <div class="min-w-0">
        <h3 class="text-[15px] font-semibold leading-snug text-slate-900">{{ $activity->name }}</h3>
        ...
        <div class="mt-1.5">
            <span class="inline-flex rounded-md px-1.5 py-0.5 text-[11px] font-semibold {{ $activity->status->colorClasses() }}">
                {{ $activity->status_label }}
            </span>
        </div>
    </div>

    <div class="mt-2.5 flex items-center gap-1.5 border-t border-slate-100 pt-2.5">
        @if ($showDriveActions && $activity->drive_folder_url)
            <a
            ...
            <button
                ...
                class="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-slate-700 active:bg-slate-200"
                x-on:click="copyToClipboard(@js($activity->drive_folder_url))"
            >
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
