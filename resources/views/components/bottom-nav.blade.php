<nav class="bottom-nav border-t border-slate-200 bg-white/95 shadow-[0_-2px_12px_rgba(15,23,42,0.05)] backdrop-blur">
    <div class="mx-auto grid max-w-3xl grid-cols-3">
        <a
            ...
            class="flex flex-col items-center gap-0.5 px-1 py-2 text-[11px] font-medium {{ request()->routeIs('dashboard') ? 'is-active text-brand-700' : 'text-slate-500' }}"
        >
        ...
        <a
            ...
            data-turbo="true"
            data-turbo-action="advance"
            class="flex flex-col items-center gap-0.5 px-1 py-2 text-[11px] font-medium {{ request()->routeIs('history') ? 'is-active text-brand-700' : 'text-slate-500' }}"
        >
            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
        ...
        <a
            ...
            data-turbo="true"
            data-turbo-action="advance"
            class="flex flex-col items-center gap-0.5 px-1 py-2 text-[11px] font-medium {{ request()->routeIs('settings.*') ? 'is-active text-brand-700' : 'text-slate-500' }}"
        >
            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
